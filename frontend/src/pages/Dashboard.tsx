...
const Dashboard: React.FC = () => {
  ...
  const countFilledFields = React.useCallback(
    (value: any): { filled: number; total: number } => {
      ...
      const walk = (node: any): { filled: number; total: number } => {
        if (node === null || node === undefined) return { filled: 0, total: 0 };
        if (Array.isArray(node)) {
          if (node.length === 0) return { filled: 0, total: 1 };
          return node.reduce(
            (acc, item) => {
              const child = walk(item);
              acc.filled += child.filled;
        ...
        return { filled: isEmptyPrimitive(node) ? 0 : 1, total: 1 };
      };
  ...
  const calcProjectProgress = React.useCallback(
    (project: Project, sections: Record<string, any>): number => {
      const parts = [
        {
          name: "project",
          value: {
        ...
        { name: "qualification", value: sections.qualification },
        { name: "capacity", value: sections.capacity },
        { name: "strategy", value: sections.strategy },
        { name: "requirements", value: sections.requirements },
        { name: "indicators", value: sections.indicators },
      ];

      let filled = 0;
      let total = 0;
      for (const part of parts) {
        const score = countFilledFields(part.value);
        filled += score.filled;
        total += score.total;
      }
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      if (projects.length === 0) {
        if (mounted) setProgressByProject({});
        return;
      }

      const results = await Promise.all(
        projects.map(async (project) => {
          const [qualification, capacity, strategy, requirements, indicators] =
            await Promise.all([
              projectService.getQualification(project.id).catch(() => null),
              projectService.getCapacity(project.id).catch(() => null),
              projectService.getStrategy(project.id).catch(() => null),
              projectService.getRequirements(project.id).catch(() => null),
              projectService.getIndicators(project.id).catch(() => null),
            ]);
      ...
      if (!mounted) return;
      setProgressByProject(Object.fromEntries(results));
    };
  ...
  const prev = () =>
    setCurrentPage((p) => (pageCount ? (p - 1 + pageCount) % pageCount : 0));
  const next = () =>
    setCurrentPage((p) => (pageCount ? (p + 1) % pageCount : 0));
  ...
  const [companiesCount, setCompaniesCount] = React.useState<number | null>(
    null,
  );
    ...
    const load = async () => {
      try {
        ...
        if (dbId) {
          ...
          const db = res?.data;
          if (db && db.role)
            master = String(db.role).toUpperCase() === "MASTER";
        } else {
          const useToken =
            token ??
            (() => {
          ...
          if (useToken) {
            ...
            const roles = extractRolesFromTokenPayload(payload).map((r) =>
  ...
  const NoForwardSpan = React.forwardRef<
    HTMLSpanElement,
    React.HTMLAttributes<HTMLSpanElement>
  >(({ children, style, ...rest }, ref) => {
    const { interactive, ...domProps } = rest as any;
    return (
      <span ref={ref} style={style} {...domProps}>
        {children}
      </span>
    );
  });
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        <Paper elevation={3} sx={{ p: 2, width: "100%", mb: 2 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
            <IconButton onClick={prev} aria-label="previous" size="large">
              <ArrowBackIosIcon />
            </IconButton>
            ...
            {projects.length === 0 ? (
              <Box sx={{ flex: 1, py: 4, textAlign: "center" }}>
                <Typography variant="body1">
                  {t("dashboard_extra_local.noProjects")}
                </Typography>
              </Box>
            ) : (
                        ...
                        title={
                          ...
                          >
                            ...
                            {[
                              ...
                              const sample =
                                [80, 55, 40, 70, 25, 60, 45, 30, 50][i] ?? 50;
                              return (
                        ...
                        <NoForwardSpan style={{ display: "inline-block" }}>
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                <CompletionDonut
                                  percent={progressByProject[proj.id] ?? 0}
                                  size={96}
                                />
                                <Typography
                                  variant="caption"
                                ...
                                >
                                  {t("dashboard_extra_local.fieldsFilled")}
                                </Typography>
                              </Box>
                            </CardContent>
                            <CardActions>
            ...
            <IconButton onClick={next} aria-label="next" size="large">
              <ArrowForwardIosIcon />
            </IconButton>
          </Box>
              ...
              {Array.from({ length: pageCount }).map((_, i) => (
                <Box
                  ...
                  sx={{
                    ...
                    height: 10,
                    borderRadius: "50%",
                    bgcolor: i === currentPage ? "primary.main" : "grey.400",
                    cursor: "pointer",
                  }}
                />
              ))}
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography variant="subtitle2">
                  {t("dashboard_extra_local.totalUsers")}
                </Typography>
              </Box>
              ...
              <Typography variant="caption" sx={{ color: "text.secondary" }}>
                {t("dashboard_extra_local.allUsersScope")}
              </Typography>
            </Paper>
          </motion.div>
          ...
          >
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Typography variant="subtitle2">
                  {t("dashboard_extra_local.activeUsers")}
                </Typography>
              </Box>
              <Typography variant="h4">
                {usersActive === null ? "..." : usersActive}
              </Typography>
              <Typography variant="caption" sx={{ color: "text.secondary" }}>
                {t("dashboard_extra_local.last30Minutes")}
              </Typography>
            </Paper>
          </motion.div>
          <motion.div
          ...
          >
            ...
            >
              <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                ...
                <Box>
                  <Typography variant="h6">
                    {t("dashboard.projectsCardTitle") || t("nav.projects")}
                  </Typography>
                </Box>
              </Box>
              ...
              >
                <Box>
                  ...
                  <Typography variant="body2" sx={{ opacity: 0.9 }}>
                    {t("dashboard.overview") || "Projects overview"}
                  </Typography>
                </Box>
      ...
      <Box sx={{ mt: 2, display: "flex", gap: 2, flexWrap: "wrap" }}>
        <Paper sx={{ p: 2, width: "40vw", maxWidth: "100%" }}>
          <Typography variant="subtitle2">
            {t("dashboard.projectsPerMonth")}
          </Typography>
          <Box sx={{ height: 220, mt: 1 }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={buckets.map((b) => ({ name: b.label, count: b.count }))}
