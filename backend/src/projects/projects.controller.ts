...
export class ProjectsController {
  ...
  async list(
    ...
    if (actor) {
      ...
      if (actor.role === Role.ADMIN) {
        const tenant =
          actor.tenantId ?? (actor.company && (actor.company as any).id);
        if (!tenant) return [];
  ...
  async getStrategy(@Param("id") id: string) {
    try {
      ...
      throw new InternalServerErrorException("Failed to load strategy data");
    }
  }
  ...
  async saveStrategy(
    ...
    try {
      return await this.projectsService.saveStrategy(id, dto, actor as any);
    } catch (e) {
