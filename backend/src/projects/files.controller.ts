...
export class ProjectFilesController {
  ...
  @Post('presign')
  async presign(@Param('projectId') projectId: string, @Body() body: { filename: string; projectName?: string; tabName?: string; fieldName?: string }) {
    const rawName = (body && body.projectName) ? String(body.projectName) : projectId
    const tabRaw = (body && body.tabName) ? String(body.tabName) : null
    const fieldRaw = (body && body.fieldName) ? String(body.fieldName) : null
    const sanitize = (s: string) => String(s || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').trim().replace(/\s+/g, '_').replace(/[^a-zA-Z0-9_-]/g, '') || projectId
  ...
  async list(@Param('projectId') projectId: string) {
    const list = await this.filesService.listByProject(projectId)
    this.logger.log(`list files for project=${projectId} count=${Array.isArray(list)?list.length:0}`)
    try {
      if (Array.isArray(list) && list.length > 0) this.logger.debug(`sample file keys: ${list.slice(0,5).map(f=>f.labelKey||f.originalName||f.s3Key).join(', ')}`)
    } catch {}
    return list
  }
